...
与其他控制台程序一样，Thu Terminal Engine不适合实时高速渲染。\n
By ATN\n
结构：\n
decorate_string()\n
len_in_size()\n
...
def decorate_string(string, fore_color='default', back_color='default'):
...
def print_in_pos(string, x, y, fore_color='default', back_color='default'):
    """在指定位置渲染字符。注意不要重复将ThuTerminalEngine.decorate_string()返回的修饰过的字符传入。\n
    string ->str 字符。\n
    x ->int x坐标。\n
    y ->int y坐标。\n
    fore_color='default' ->str 前景色。\n
    ...
    while y < max_y - 1:
        ...
        while x < max_x - 1:
            ...
            for n in range(0, len(string_list)):
                ...
                if s[1] == x and s[2] == y:
                    now_print = s[0]
                    if x + len_in_size(now_print) > max_x:
                        if len(now_print) == 1: #中文字符
                            now_print = decorate_string(' ', colors[0], colors[1])
                        elif len(now_print) == 2: #英文字符
                            now_print = decorate_string(now_print[0], colors[0], colors[1])
                    final_string += decorate_string(now_print, colors[0], colors[1])
                    printed = True
                    x += len_in_size(now_print) - 1
                    break
            if not printed:
                final_string += ' '
    if final_string[-1] == '\n':
...
def clear():
    """清空界面。默认在ThuTerminalEngine.update()中调用。"""
    global string_list
    string_list = []

if __name__ == '__main__':
    os.system('cls')
    background = '欢迎使用ThuTerminalEngine。'
    while 1:
        print_in_pos(background, int(get_terminal_size()[0] / 2) - int(len_in_size(background) / 2), int(get_terminal_size()[1] / 2))
        update()
        time.sleep(0.01)
